...
public unsafe partial struct NSplitMapCtrl<K>
{
    ...
    private void Resize(int new_size)
    {
        ...
        {
            ref var entry = ref m_ctrls.m_items[i];
            if (entry.Next >= -1)
            {
    ...
    public InsertResult TryInsert(K key, bool overwrite, ref CtrlOp op, out int idx)
    {
        ...
        {
            index = m_free_list;
            Debug.Assert((StartOfFreeList - m_ctrls.m_items[m_free_list].Next) >= -1, "shouldn't overflow because `next` cannot underflow");
            m_free_list = StartOfFreeList - m_ctrls.m_items[m_free_list].Next;
            m_free_count--;
        }
        else
        {
        ...
        {
            idx = index;
            ref var entry = ref m_ctrls.m_items[index];
    ...
    public int Remove(K key)
    {
        ...
        while (i >= 0)
        {
            ...
            if (entry.HashCode == hash_code && EqualityComparer<K>.Default.Equals(key, entry.Key))
            {
                ...
                {
                    m_ctrls.m_items[last].Next = entry.Next;
                }
    ...
    public ref struct Enumerator(ref readonly NSplitMapCtrl<K> self)
    {
        ...
        public bool MoveNext()
        {
            ...
            {
                var i = index++;
                ref var entry = ref self.m_ctrls.m_items[i];
