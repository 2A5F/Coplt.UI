...
void Coplt::coplt_ui_dwrite_get_font_face_info(IDWriteFontFace5* face, NFontInfo* info)
{
    ...
    info->Metrics.Ascent = static_cast<float>(metrics.ascent);
    info->Metrics.Descent = static_cast<float>(metrics.descent);
    info->Metrics.Leading = static_cast<float>(metrics.lineGap);
    info->Metrics.LineHeight = static_cast<float>(metrics.ascent + metrics.descent + metrics.lineGap);
    info->Metrics.UnitsPerEm = metrics.designUnitsPerEm;
